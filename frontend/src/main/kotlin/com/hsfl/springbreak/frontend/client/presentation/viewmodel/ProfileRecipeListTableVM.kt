...
class ProfileRecipeListTableVM(
    ...
    fun onEvent(event: LifecycleEvent) {
        when (event) {
            LifecycleEvent.OnMount -> fetchMyRecipe()
    ...
    private fun fetchMyRecipe() = scope.launch {
        recipeRepository.getMyRecipes(userState.userState.value.id).collectLatest { response ->
