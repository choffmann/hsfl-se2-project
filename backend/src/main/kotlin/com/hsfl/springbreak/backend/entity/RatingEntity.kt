...
data class RatingEntity(
    ...
    fun toDto(): Rating = Rating(
        id = this.id!!, stars = this.stars, recipe = this.recipe.toDto(), user = this.user.toDto()
    )
    ...
    companion object {
        ...
        fun fromDto(newRating: Rating.SendRating, recipe: RecipeEntity, user: UserEntity): RatingEntity = RatingEntity(
            stars = newRating.stars, recipe = recipe, user = user
        )
    }
}
